...
export function SetupWizard() {
  ...
  const [lastFailedMode, setLastFailedMode] = useState<string | null>(null);
    ...
    getSetupStatus()
      .then((status) => {
        if (status.setup_complete) {
          ...
          status.identity_mode !== "not_exposed"
        ) {
          setIdentityMode(status.identity_mode);
          ...
          if (status.identity_mode === "attach_account" && pendingAuth) {
            try {
              const payload = JSON.parse(pendingAuth) as {
                attachedDid: string;
              };
    ...
    if (mode === "not_exposed") {
      try {
        ...
        setError(
          e instanceof Error
            ? e.message
            : "Failed to save configuration. Check that your backend is running and try again.",
        );
      }
      ...
      try {
        ...
        setError(
          e instanceof Error
            ? e.message
            : "Failed to configure identity. Check that your backend is running and try again.",
        );
      }
    }
  }, []);
  ...
  const handleGoBack = useCallback(() => {
    ...
    switch (currentStep) {
      ...
      case "verify":
        setCurrentStep(
          identityMode === "attach_account" ? "configure" : "mode",
        );
        break;
    }
  ...
  const handleConfigureComplete = useCallback(
    (opts?: { attachedDid?: string; attachedHandle?: string | null }) => {
      if (opts?.attachedDid) {
  ...
  const handleStepperNav = useCallback(
    (value: string) => {
  ...
  return (
    <Stepper value={currentStep} onValueChange={handleStepperNav}>
        ...
        {identityMode === "attach_account" && (
          <>
            ...
            <StepperItem value="attach-auth">
              <StepperTrigger>
                <StepperIndicator />
                <StepperTitle>Sign In</StepperTitle>
              </StepperTrigger>
              <StepperSeparator />
            </StepperItem>
          </>
      ...
      {error && (
        <div
          role="alert"
          className="mt-4 rounded-lg border border-destructive/20 bg-destructive/10 p-3 text-sm text-destructive flex items-center justify-between gap-3"
        >
          <span>{error}</span>
          {lastFailedMode && (
            <Button
              ...
              onClick={() => {
                if (lastFailedMode) handleModeSelected(lastFailedMode);
              }}
            >
              Try again
            </Button>
      ...
      <div ref={stepContentRef} tabIndex={-1} className="mt-8 outline-none">
        {currentStep === "mode" && (
          <SetupIdentityMode onComplete={handleModeSelected} />
        )}
